...
const myAbi =
  require("../../artifacts/contracts/MyContracts.sol/MyContract.json").abi;
...
const main = async function () {
  const contractIsDeployed =
    (await provider.getCode(myContractAddress)) !== "0x";

  if (!contractIsDeployed) {
    throw Error(
      "MyContract is not deployed! Deploy it by running the deploy script."
    );
  }
  ...
  let balanceOfUnderlying =
    (await cToken.callStatic.balanceOfUnderlying(myContractAddress)) /
  ...
  console.log(
    `MyContract's c${assetName} Token Balance:`,
    +cTokenBalance / 1e8
  );
  ...
  tx = await myContract.redeemCErc20Tokens(amount, redeemType, cTokenAddress);
